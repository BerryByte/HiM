...
class CustomCentripetalTripletLoss(torch.nn.Module):
    r"""
    A modified centripetal triplet loss that compares the hyperbolic norms
    (distance from origin) using a logarithmic transformation.

    It enforces:
      log(d(child, 0)) > log(d(parent, 0)) + margin.
    """

    def __init__(self, manifold,margin: float, eps: float = 1e-7):
        super().__init__()
        self.margin = margin
        self.eps = eps
    ...
    def get_config_dict(self):
        try:
            ...
            config = {
                "distance_metric": f"Lorentz(c={self.manifold.k}).dist",
                "margin": self.margin,
            }
        return config
    def forward(self, rep_anchor: torch.Tensor, rep_positive: torch.Tensor, rep_negative: torch.Tensor):
        # Compute hyperbolic distances to origin (using manifold.dist0)
        norm_anchor = self.manifold.dist0(rep_anchor)  # child
        norm_positive = self.manifold.dist0(rep_positive)  # parent
        # Use log of the norms so that even small absolute differences become noticeable.
        loss = F.relu(self.margin + torch.log(norm_positive + self.eps) - torch.log(norm_anchor + self.eps))
        return loss.mean()
