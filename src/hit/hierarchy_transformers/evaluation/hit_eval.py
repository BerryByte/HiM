...
class HierarchyTransformerEvaluator(HierarchyEvaluator):
    """Evaluator for hierarchy-trained language models with metrics logging support."""
    ...
    def encode(model: HierarchyTransformer, examples: list, eval_batch_size: int):
        child_embeds = model.encode([x.texts[0] for x in examples], convert_to_tensor=True,show_progress_bar=False)
        parent_embeds = model.encode([x.texts[1] for x in examples], convert_to_tensor=True,show_progress_bar=False)
        labels = torch.tensor([x.label for x in examples]).to(child_embeds.device)
        dists = model.manifold.dist(child_embeds, parent_embeds).squeeze()
        child_norms = model.manifold.dist0(child_embeds).squeeze()
        parent_norms = model.manifold.dist0(parent_embeds).squeeze()
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        try:
            ...
            print("Error occurred while evaluating on test examples.")
            return 0.0
